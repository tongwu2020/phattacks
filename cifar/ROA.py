...
class ROA(object):
    ...
    def exhaustive_search(self, X, y, alpha, num_iter, width, height, xskip, yskip,random = False):
        ...
        with torch.set_grad_enabled(True):
            return self.inside_pgd(X,y,width, height,alpha, num_iter, xskip, yskip, output_j, output_i ,random)
    ...
    def inside_pgd(self, X, y, width, height, alpha, num_iter, xskip, yskip, out_j, out_i, random = False):
        ...
        for t in range(num_iter):
            ...
            #imshow("test"+str(t),X1.detach())
            X1.grad.zero_()
        #imshow("test2",X1.detach())
        return (X1).detach()
