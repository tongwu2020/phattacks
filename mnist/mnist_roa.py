...
parser.add_argument('--inputmodel', type=str, default="ori", help='1 == plot all successful adversarial images')
parser.add_argument('--modelname', type=str, help='1 == plot all successful adversarial images')
...
# Set up the model
model=CNN()
if opt.inputmodel == "ori":
    model.apply(weights_init)
else:
    model.load_state_dict(torch.load('./model/'+ opt.inputmodel + '.pt'))
...
for epoch in range(epochs):
    for i, (images,labels) in enumerate(train_load):
        ...
        images = roa.gradient_based_search(images, labels, learning_rate,\
...
torch.save(model.state_dict(), './model/'+opt.modelname +'.pt')  
print("Finished!")
